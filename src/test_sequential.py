...
def main_worker(local_rank, args):
    ...
    for i, info in enumerate(all_info):
        gt[i] = [info['target_item']] #[int(info['target_item'])]
        pred_dict = {}
        for j in range(len(info['gen_item_list'])):
            try:
                pred_dict[info['gen_item_list'][j]] = -(j+1) #pred_dict[int(info['gen_item_list'][j])] = -(j+1)
            except:
                pass
    ...
    if args.distributed:
        ...
        save_path = os.path.join(args.train_url, 'result_' + args.test_prompt + '.json')
        with open(save_path, "w") as f:
            json.dump({'ui_scores': ui_scores, 'gt': gt}, f)
