...
class P5_Amazon_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if task_name == 'sequential':
            ...
            if self.split == 'books' or self.split == 'amazon-books':
                ...
                if task_template['id'] in task_temp_seq:
                    ...
                    source_text = task_template['source'].format(user_info, ' , '.join(purchase_history))
            elif self.split.startswith('Foursquare_') or self.split == 'gowalla':
                task_temp = ['2-4', '2-9', '2-10']
                task_temp_seq = ['2-3', '2-5', '2-7']
                
                if task_template['id'] not in task_temp :
                    user_info = user_id # user id
                else:
                    user_info = user_desc #user name
                ...
                assert 0, 'unknown split {}'.format(self.split)
...
def get_loader(args, task_list, sample_numbers, split='toys', mode='train', 
    ...
    if split == 'books' or split == 'amazon-books':
        from mask_amazon_templates import all_tasks as task_templates
    elif split == 'movielens' or split == 'ml-10m':
    ...
    elif split.startswith('Foursquare_') or split == 'gowalla':
        from mask_Foursquare_tsmc2024_NYC_templates import all_tasks as task_templates
    else:
        assert 0, 'unknown split {}'.format(split)
