...
def data_item():
    ...
    with open('pid2idx.json', encoding='utf-8') as f:
        ...
        for pid in poi_info.keys():
            ...
            if int(k) in is_in:    
                eid = 'p'+k
                m = pat.match(poi_info[pid]['poi'])
                if m:
                    a = m.groups()
                assert len(a) == 2, poi_info[pid]['poi']
                add, cat = a[0], a[1]
                ...
                triple[eid] = {'poi.poi.category':[category[cat]], 'poi.poi.address':[address[add2]], 'poi.poi.lat_lon':[lat_lon[lat_lon_]]}
    
    print(f'# of category {len(category)}, address {len(address)}, latlon {len(lat_lon)}')
