...
def data_item():
    ...
    for uname in user_id2name.values():
        ...
        id2item[uname] = uname
        if uid not in user_comm:
            assert 0, f'{uid} not found in community'
        else:
            comm = str(user_comm[uid])
            found += 1

        if comm not in community:
            cid = 'com'+comm
            entity2text[cid] = comm
            community[comm] = cid
        
        triple[uname] = {'poi.user.community':[community[comm]]}

    print(f'# of category {len(category)}, address {len(address)} and community {len(community)}')

    # # poi 1312372
    # # of category 441, address 629 and community 329
